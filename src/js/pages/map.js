...
document.addEventListener(`DOMContentLoaded`, () => {
  ...
  _0.els(`[data-pressed]`).forEach((_target) => {
    _target.addEventListener(`click`, () => {
      if (_target.closest(`[data-groupWrap].slideTab`)) {
        _target.setAttribute(`data-pressed`, true); // slideTab은 해제 불가능
      } else {
        _target.setAttribute(
          `data-pressed`,
          _target.getAttribute(`data-pressed`) == `true` ? `false` : `true`
        );
      }
      ...
      if (_target.getAttribute(`data-group`)) {
        _target.parentElement
          .closest(`[data-groupWrap`)
          .querySelectorAll(
            `[data-group="${_target.getAttribute(`data-group`)}"]`
          )
          .forEach((_groupItem) => {
            if (_groupItem != _target)
              _groupItem.setAttribute(`data-pressed`, `false`);
          });
      }
    });
  });
  ...
  _0.els(`input[type=radio][data-target]`).forEach((_tab) => {
    _tab.addEventListener(`click`, () => {
      _tab
        .closest(`[data-groupwrap]`)
        .querySelectorAll(`input[name='${_tab.getAttribute(`name`)}']`)
        .forEach((_siblings) => {
          _0.el(
            `[data-title='${_siblings.getAttribute(`data-target`)}']`
          ).setAttribute(`data-show`, `false`);
        });
      _0.el(`[data-title='${_tab.getAttribute(`data-target`)}']`).setAttribute(
        `data-show`,
        `true`
      );
    });
  ...
  // result 펴고 접기
  _0.el(`section.list button.folding.show`).addEventListener(`click`, () =>
    _0.result.show()
  );
  ...
  _0.els(`section.list nav.filter button`).forEach((_button) => {
    _button.addEventListener(`click`, () => {
      if (_button.getAttribute(`data-pressed`) == `true`) {
        ...
        _0.els(`.filterWrap .set`).forEach((_set) =>
          _set.setAttribute(`data-show`, `false`)
        );
        _0.el(
          `.filterWrap .set[data-title="${_button.getAttribute(
            `data-target`
          )}"]`
        ).setAttribute(`data-show`, _button.getAttribute(`data-pressed`));
        _0.result.show();
      } else {
  ...
  _0.els(`[data-selected]`).forEach((_target) => {
    _target.addEventListener(`click`, () => {
      _0.els(`[data-selectWrap] [data-selected]`).forEach((_siblings) => {
        if (_siblings != _target)
          _siblings.setAttribute(`data-selected`, `false`);
      });
      _target.setAttribute(
        `data-selected`,
        _target.getAttribute(`data-selected`) == `true` ? `false` : `true`
      );
      _target.getAttribute(`data-selected`) == `true`
        ? _0.detail.show()
        : _0.detail.hide();
    });
  });
  ...
  _0.els(`aside.sidebar button[aria-haspopup="true"]`).forEach((_button) => {
    _button.addEventListener(`click`, () => {
      _0.els(`aside.sidebar .popup`).forEach((_popup) =>
        _popup.setAttribute(`data-show`, false)
      );
      _0.el(
        `aside.sidebar .popup[data-title="${_button.getAttribute(
          `data-target`
        )}"]`
      ).setAttribute(`data-show`, _button.getAttribute(`data-pressed`));
    });
  ...
  _0.els(`aside.sidebar .popup button.close`).forEach((_button) => {
    _button.addEventListener(`click`, () =>
      _0
        .el(
          `aside.sidebar button.menu[data-target='${_button
            .closest(`.popup`)
  ...
  _0.els(`.modal header button.close`).forEach((_button) => {
    _button.addEventListener(`click`, () =>
      _button.closest(`.modal`).setAttribute(`data-show`, `false`)
    );
  });
});
...
const _0 = {
  ...
  filter: {
    reset: function () {
      _0.els(`nav.filter button`).forEach(
        (_button) => (_button.disabled = false)
      );
      _0.el(`nav.filter button[data-pressed='true']`).classList.remove(
        `applyed`
      );
      _0.el(`nav.filter button[data-pressed='true']`).setAttribute(
        `data-pressed`,
        `false`
      );
      _0.el(`.filterWrap`).setAttribute(`data-folded`, `true`);
  ...
  sample: {
    filterApply: function () {
      ...
      _0.els(`nav.filter button`).forEach(
        (_button) => (_button.disabled = true)
      );
      _0.el(`nav.filter button[data-pressed='true']`).classList.add(`applyed`);
      _0.el(
        `nav.filter button[data-pressed='true'] span`
      ).textContent = `선택사항`;
    },
