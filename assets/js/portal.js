    ...
    init() {
      ...
      if (dateMonthInputs.length) {
        dateMonthInputs.forEach((input) => {
          ...
          const defaultValue = input.value && /^\d{4}\.\d{2}$/.test(input.value) ? input.value.replace(".", "-") + "-01" : null;
          flatpickr(input, {
            dateFormat: "Y.m",
            locale: "ko",
            allowInput: false,
            defaultDate: defaultValue ? new Date(defaultValue) : void 0,
      ...
      if (dateYearInputs.length) {
        dateYearInputs.forEach((input) => {
          ...
          flatpickr(input, {
            ...
            allowInput: false,
            clickOpens: true,
            defaultDate: defaultYear ? new Date(defaultYear, 0, 1) : void 0,
            onReady(selectedDates, dateStr, instance) {
