...
document.addEventListener(`DOMContentLoaded`, () => {
  ...
  _0.els(`[data-pressed]`).forEach((_target) => {
    _target.addEventListener(`click`, () => {
      ...
      if (_target.getAttribute(`data-group`)) {
        _target.parentElement.closest(`[data-groupWrap`).querySelectorAll(
  ...
  _0.el(`section.list button.folding.show`).addEventListener(
    `click`,
    () => _0.result.show()
  );
  ...
  _0.els(`section.list nav.filter button`).forEach((_button) => {
    _button.addEventListener(`click`, () => {
      if (_button.getAttribute(`data-pressed`) == `true`) {
        ...
        _0.els(`.filterWrap .set`).forEach(
          (_set) => _set.setAttribute(`data-show`, `false`)
        );
        _0.el(
  ...
  _0.els(`aside.sidebar button[aria-haspopup="true"]`).forEach((_button) => {
    _button.addEventListener(`click`, () => {
      _0.els(`aside.sidebar .popup`).forEach(
        (_popup) => _popup.setAttribute(`data-show`, false)
      );
      _0.el(
  ...
  _0.els(`aside.sidebar .popup button.close`).forEach((_button) => {
    _button.addEventListener(
      `click`,
      () => _0.el(
        `aside.sidebar button.menu[data-target='${_button.closest(`.popup`).getAttribute(`data-title`)}']`
      ).click()
    );
  });
